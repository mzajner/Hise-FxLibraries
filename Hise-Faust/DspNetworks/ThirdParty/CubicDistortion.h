...
#include "src/CubicDistortion.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct CubicDistortionMetaData {
		SN_NODE_ID("CubicDistortion");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using CubicDistortion = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _CubicDistortion, CubicDistortionMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

...
#include "src/MicroShifter.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct MicroShifterMetaData {
		SN_NODE_ID("MicroShifter");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using MicroShifter = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _MicroShifter, MicroShifterMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

...
#include "src/TransientShaper.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct TransientShaperMetaData {
		SN_NODE_ID("TransientShaper");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using TransientShaper = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _TransientShaper, TransientShaperMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

...
#include "src/QuadraticDistortion.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct QuadraticDistortionMetaData {
		SN_NODE_ID("QuadraticDistortion");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using QuadraticDistortion = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _QuadraticDistortion, QuadraticDistortionMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

...
#include "src/SallenKeyBpf.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct SallenKeyBpfMetaData {
		SN_NODE_ID("SallenKeyBpf");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using SallenKeyBpf = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _SallenKeyBpf, SallenKeyBpfMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

...
#include "src/randomRingModulation.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct randomRingModulationMetaData {
		SN_NODE_ID("randomRingModulation");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using randomRingModulation = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _randomRingModulation, randomRingModulationMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

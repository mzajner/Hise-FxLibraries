...
#include "src/CrossStereoDelay.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct CrossStereoDelayMetaData {
		SN_NODE_ID("CrossStereoDelay");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using CrossStereoDelay = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _CrossStereoDelay, CrossStereoDelayMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

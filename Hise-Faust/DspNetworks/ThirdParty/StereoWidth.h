...
#include "src/StereoWidth.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct StereoWidthMetaData {
		SN_NODE_ID("StereoWidth");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using StereoWidth = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _StereoWidth, StereoWidthMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

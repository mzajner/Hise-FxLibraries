...
#include "src/DiodeLadderLpf4Poles.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct DiodeLadderLpf4PolesMetaData {
		SN_NODE_ID("DiodeLadderLpf4Poles");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using DiodeLadderLpf4Poles = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _DiodeLadderLpf4Poles, DiodeLadderLpf4PolesMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

...
#include "src/mooghalfladder.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct mooghalfladderMetaData {
		SN_NODE_ID("mooghalfladder");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using mooghalfladder = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _mooghalfladder, mooghalfladderMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

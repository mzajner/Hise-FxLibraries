...
Compilation options: -lang cpp -fpga-mem-th 4 -rui -nvi -ct 1 -cn _oberheimLPF -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0
------------------------------------------------------------ */

#ifndef  ___oberheimLPF_H__
#define  ___oberheimLPF_H__
...
#ifndef FAUSTCLASS 
#define FAUSTCLASS _oberheimLPF
#endif
...
static float _oberheimLPF_faustpower2_f(float value) {
	return value * value;
}

struct _oberheimLPF final : public ::faust::dsp {
	...
	_oberheimLPF(const _oberheimLPF&) = default;
	
	virtual ~_oberheimLPF() = default;
	
	_oberheimLPF& operator=(const _oberheimLPF&) = default;
	
	void metadata(Meta* m) { 
		m->declare("author", "Eric Tarr");
		m->declare("compile_options", "-lang cpp -fpga-mem-th 4 -rui -nvi -ct 1 -cn _oberheimLPF -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0");
		m->declare("description", "Demonstration of the Oberheim Low-Pass Filter");
		m->declare("filename", "oberheimLPF.dsp");
	...
	void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("freq", &fHslider0, FAUSTFLOAT(1.0f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.001f));
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp6 = 1.0f - 0.33333334f * _oberheimLPF_faustpower2_f(fTemp5);
			...
			float fTemp12 = 1.0f - 0.33333334f * _oberheimLPF_faustpower2_f(fTemp11);
			float fTemp13 = fTemp0 * fTemp11 * fTemp12;
			float fRec5 = fRec6[1] + fTemp13;
			fRec6[0] = fRec6[1] + 2.0f * fTemp13;
			float fTemp14 = fTemp11 * fTemp12;
			fRec7[0] = fTemp10 + fTemp14;
	...
	#define FAUST_FILE_NAME "oberheimLPF.dsp"
	#define FAUST_CLASS_NAME "_oberheimLPF"
	#define FAUST_COMPILATION_OPIONS "-lang cpp -fpga-mem-th 4 -rui -nvi -ct 1 -cn _oberheimLPF -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0"
	...
	FAUST_ADDHORIZONTALSLIDER("freq", fHslider0, 1.0f, 0.0f, 1.0f, 0.001f);

	#define FAUST_LIST_ACTIVES(p) \
		p(HORIZONTALSLIDER, Q, "Q", fHslider1, 1.0f, 0.5f, 1e+01f, 0.01f) \
		p(HORIZONTALSLIDER, freq, "freq", fHslider0, 1.0f, 0.0f, 1.0f, 0.001f) \

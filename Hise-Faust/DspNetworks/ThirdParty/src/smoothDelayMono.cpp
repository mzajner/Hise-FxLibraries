...
Compilation options: -lang cpp -fpga-mem-th 4 -rui -nvi -ct 1 -cn _smoothDelayMono -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0
------------------------------------------------------------ */

#ifndef  ___smoothDelayMono_H__
#define  ___smoothDelayMono_H__
...
#ifndef FAUSTCLASS 
#define FAUSTCLASS _smoothDelayMono
#endif
...
struct _smoothDelayMono final : public ::faust::dsp {
	...
	_smoothDelayMono(const _smoothDelayMono&) = default;
	
	virtual ~_smoothDelayMono() = default;
	
	_smoothDelayMono& operator=(const _smoothDelayMono&) = default;
	
	void metadata(Meta* m) { 
		m->declare("author", "Yann Orlarey");
		m->declare("basics.lib/name", "Faust Basic Element Library");
		m->declare("basics.lib/version", "1.22.0");
		m->declare("compile_options", "-lang cpp -fpga-mem-th 4 -rui -nvi -ct 1 -cn _smoothDelayMono -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0");
		m->declare("copyright", "Grame");
		m->declare("delays.lib/name", "Faust Delay Library");
		m->declare("delays.lib/version", "1.2.0");
		m->declare("filename", "smoothDelayMono.dsp");
	...
	_smoothDelayMono* clone() {
		return new _smoothDelayMono(*this);
	}
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow1 = fConst1 * std::max<float>(0.0f, std::min<float>(5e+03f, static_cast<float>(fHslider1)));
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp2 = ((fRec1[1] != 0.0f) ? (((fRec2[1] > 0.0f) & (fRec2[1] < 1.0f)) ? fRec1[1] : 0.0f) : (((fRec2[1] == 0.0f) & (fSlow1 != fRec3[1])) ? fTemp1 : (((fRec2[1] == 1.0f) & (fSlow1 != fRec4[1])) ? -fTemp1 : 0.0f)));
			fRec1[0] = fTemp2;
			...
			float fTemp3 = fVec0[(IOTA0 - static_cast<int>(std::min<float>(524288.0f, std::max<float>(0.0f, fRec3[0])))) & 1048575];
			fRec0[0] = fTemp3 + fRec2[0] * (fVec0[(IOTA0 - static_cast<int>(std::min<float>(524288.0f, std::max<float>(0.0f, fRec4[0])))) & 1048575] - fTemp3);
	...
	#define FAUST_FILE_NAME "smoothDelayMono.dsp"
	#define FAUST_CLASS_NAME "_smoothDelayMono"
	#define FAUST_COMPILATION_OPIONS "-lang cpp -fpga-mem-th 4 -rui -nvi -ct 1 -cn _smoothDelayMono -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0"

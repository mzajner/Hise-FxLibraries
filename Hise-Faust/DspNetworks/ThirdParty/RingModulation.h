...
#include "src/RingModulation.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct RingModulationMetaData {
		SN_NODE_ID("RingModulation");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using RingModulation = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _RingModulation, RingModulationMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

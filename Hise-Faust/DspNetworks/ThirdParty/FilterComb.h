...
#include "src/FilterComb.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct FilterCombMetaData {
		SN_NODE_ID("FilterComb");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using FilterComb = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _FilterComb, FilterCombMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

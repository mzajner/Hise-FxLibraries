...
#include "src/Pixelate_Broken.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct Pixelate_BrokenMetaData {
		SN_NODE_ID("Pixelate_Broken");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using Pixelate_Broken = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _Pixelate_Broken, Pixelate_BrokenMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

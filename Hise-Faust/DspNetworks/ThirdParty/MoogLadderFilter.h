...
#include "src/MoogLadderFilter.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct MoogLadderFilterMetaData {
		SN_NODE_ID("MoogLadderFilter");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using MoogLadderFilter = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _MoogLadderFilter, MoogLadderFilterMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

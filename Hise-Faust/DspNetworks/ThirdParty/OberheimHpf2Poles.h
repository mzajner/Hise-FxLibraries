...
#include "src/OberheimHpf2Poles.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct OberheimHpf2PolesMetaData {
		SN_NODE_ID("OberheimHpf2Poles");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using OberheimHpf2Poles = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _OberheimHpf2Poles, OberheimHpf2PolesMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

...
#include "src/SallenKeyHpf.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct SallenKeyHpfMetaData {
		SN_NODE_ID("SallenKeyHpf");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using SallenKeyHpf = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _SallenKeyHpf, SallenKeyHpfMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

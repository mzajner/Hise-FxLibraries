...
#include "src/PowerAmp.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct PowerAmpMetaData {
		SN_NODE_ID("PowerAmp");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using PowerAmp = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _PowerAmp, PowerAmpMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

...
#include "src/autoPanner.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct autoPannerMetaData {
		SN_NODE_ID("autoPanner");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using autoPanner = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _autoPanner, autoPannerMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

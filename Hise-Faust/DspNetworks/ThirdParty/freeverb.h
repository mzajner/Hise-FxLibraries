...
#include "src/freeverb.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct freeverbMetaData {
		SN_NODE_ID("freeverb");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using freeverb = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _freeverb, freeverbMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

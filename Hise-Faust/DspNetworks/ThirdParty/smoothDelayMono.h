...
#include "src/smoothDelayMono.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct smoothDelayMonoMetaData {
		SN_NODE_ID("smoothDelayMono");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using smoothDelayMono = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _smoothDelayMono, smoothDelayMonoMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

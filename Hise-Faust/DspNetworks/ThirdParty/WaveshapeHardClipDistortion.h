...
#include "src/WaveshapeHardClipDistortion.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct WaveshapeHardClipDistortionMetaData {
		SN_NODE_ID("WaveshapeHardClipDistortion");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using WaveshapeHardClipDistortion = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _WaveshapeHardClipDistortion, WaveshapeHardClipDistortionMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

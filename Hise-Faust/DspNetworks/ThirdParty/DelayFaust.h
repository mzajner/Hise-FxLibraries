...
#include "src/DelayFaust.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct DelayFaustMetaData {
		SN_NODE_ID("DelayFaust");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using DelayFaust = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _DelayFaust, DelayFaustMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

...
#include "src/RingModulationStereo.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct RingModulationStereoMetaData {
		SN_NODE_ID("RingModulationStereo");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using RingModulationStereo = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _RingModulationStereo, RingModulationStereoMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

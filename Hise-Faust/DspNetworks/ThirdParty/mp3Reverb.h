...
#include "src/mp3Reverb.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct mp3ReverbMetaData {
		SN_NODE_ID("mp3Reverb");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using mp3Reverb = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _mp3Reverb, mp3ReverbMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

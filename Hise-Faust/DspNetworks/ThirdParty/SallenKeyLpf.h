...
#include "src/SallenKeyLpf.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct SallenKeyLpfMetaData {
		SN_NODE_ID("SallenKeyLpf");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using SallenKeyLpf = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _SallenKeyLpf, SallenKeyLpfMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

...
#include "src/MoogHalfLadderFilter.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct MoogHalfLadderFilterMetaData {
		SN_NODE_ID("MoogHalfLadderFilter");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using MoogHalfLadderFilter = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _MoogHalfLadderFilter, MoogHalfLadderFilterMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

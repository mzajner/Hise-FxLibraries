...
#include "src/SaturatorWavefolding.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct SaturatorWavefoldingMetaData {
		SN_NODE_ID("SaturatorWavefolding");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using SaturatorWavefolding = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _SaturatorWavefolding, SaturatorWavefoldingMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

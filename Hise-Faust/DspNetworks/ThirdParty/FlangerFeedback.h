...
#include "src/FlangerFeedback.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct FlangerFeedbackMetaData {
		SN_NODE_ID("FlangerFeedback");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using FlangerFeedback = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _FlangerFeedback, FlangerFeedbackMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

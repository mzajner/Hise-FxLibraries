...
#include "src/reverbDattorro.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct reverbDattorroMetaData {
		SN_NODE_ID("reverbDattorro");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using reverbDattorro = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _reverbDattorro, reverbDattorroMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

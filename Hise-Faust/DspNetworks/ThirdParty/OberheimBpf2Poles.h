...
#include "src/OberheimBpf2Poles.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct OberheimBpf2PolesMetaData {
		SN_NODE_ID("OberheimBpf2Poles");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using OberheimBpf2Poles = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _OberheimBpf2Poles, OberheimBpf2PolesMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

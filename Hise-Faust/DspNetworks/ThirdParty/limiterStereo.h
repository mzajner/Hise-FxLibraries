...
#include "src/limiterStereo.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct limiterStereoMetaData {
		SN_NODE_ID("limiterStereo");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using limiterStereo = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _limiterStereo, limiterStereoMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

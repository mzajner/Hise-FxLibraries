...
Compilation options: -lang cpp -fpga-mem-th 4 -rui -nvi -ct 1 -cn _GateMono -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0
------------------------------------------------------------ */

#ifndef  ___GateMono_H__
#define  ___GateMono_H__
...
struct _GateMono final : public ::faust::dsp {
	...
	_GateMono(const _GateMono&) = default;
	
	virtual ~_GateMono() = default;
	
	_GateMono& operator=(const _GateMono&) = default;
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-lang cpp -fpga-mem-th 4 -rui -nvi -ct 1 -cn _GateMono -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0");
		m->declare("filename", "GateMono.dsp");
	...
	void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("GateMono");
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp1 = std::min<float>(fRec2[0], fRec3[0]);
			int iTemp2 = std::fabs(fTemp1) < 1.1920929e-07f;
			float fTemp3 = ((iTemp2) ? 0.0f : std::exp(-(fConst3 / ((iTemp2) ? 1.0f : fTemp1))));
			fRec1[0] = std::fabs(fTemp0) * (1.0f - fTemp3) + fRec1[1] * fTemp3;
			fRec4[0] = fSlow2 + fConst2 * fRec4[1];
			int iTemp4 = fRec1[0] > std::pow(1e+01f, 0.05f * fRec4[0]);
			iVec0[0] = iTemp4;
			fRec6[0] = fSlow3 + fConst2 * fRec6[1];
			iRec5[0] = std::max<int>((iTemp4 < iVec0[1]) * static_cast<int>(fConst0 * fRec6[0]), iRec5[1] + -1);
			float fTemp5 = std::fabs(std::max<float>(static_cast<float>(iTemp4), static_cast<float>(iRec5[0] > 0)));
			int iTemp6 = std::fabs(fRec3[0]) < 1.1920929e-07f;
			int iTemp7 = std::fabs(fRec2[0]) < 1.1920929e-07f;
			float fTemp8 = ((fTemp5 > fRec0[1]) ? ((iTemp7) ? 0.0f : std::exp(-(fConst3 / ((iTemp7) ? 1.0f : fRec2[0])))) : ((iTemp6) ? 0.0f : std::exp(-(fConst3 / ((iTemp6) ? 1.0f : fRec3[0])))));
			fRec0[0] = fTemp5 * (1.0f - fTemp8) + fRec0[1] * fTemp8;
			float fTemp9 = fTemp0 * fRec0[0];
	...
	#define FAUST_FILE_NAME "GateMono.dsp"
	#define FAUST_CLASS_NAME "_GateMono"
	#define FAUST_COMPILATION_OPIONS "-lang cpp -fpga-mem-th 4 -rui -nvi -ct 1 -cn _GateMono -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0"

...
Compilation options: -lang cpp -fpga-mem-th 4 -rui -nvi -ct 1 -cn _FuzzTone -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0
------------------------------------------------------------ */
...
struct _FuzzTone final : public ::faust::dsp {
	...
	_FuzzTone(const _FuzzTone&) = default;
	
	virtual ~_FuzzTone() = default;
	
	_FuzzTone& operator=(const _FuzzTone&) = default;
	
	void metadata(Meta* m) { 
		m->declare("author", "Bram de Jong (from musicdsp.org)");
		m->declare("compile_options", "-lang cpp -fpga-mem-th 4 -rui -nvi -ct 1 -cn _FuzzTone -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0");
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow0 = std::max<float>(0.0f, std::min<float>(1e+02f, static_cast<float>(fHslider1)));
		float fSlow1 = std::pow(1e+01f, 0.05f * std::max<float>(-96.0f, std::min<float>(96.0f, static_cast<float>(fHslider0)))) / std::sqrt(fSlow0 + 1.0f);
		float fSlow2 = std::pow(1e+01f, 0.05f * fSlow0);
		float fSlow3 = fSlow2 + -1.0f;
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			float fTemp0 = static_cast<float>(input0[i0]);
			float fTemp1 = std::fabs(fTemp0);
			output0[i0] = static_cast<FAUSTFLOAT>(fSlow1 * (fTemp0 * (fSlow2 + fTemp1) / (_FuzzTone_faustpower2_f(fTemp0) + fSlow3 * fTemp1 + 1.0f)));
			float fTemp2 = static_cast<float>(input1[i0]);
			float fTemp3 = std::fabs(fTemp2);
			output1[i0] = static_cast<FAUSTFLOAT>(fSlow1 * (fTemp2 * (fSlow2 + fTemp3) / (_FuzzTone_faustpower2_f(fTemp2) + fSlow3 * fTemp3 + 1.0f)));
		}
	}
	...
	#define FAUST_COMPILATION_OPIONS "-lang cpp -fpga-mem-th 4 -rui -nvi -ct 1 -cn _FuzzTone -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0"

...
#include "src/SpeakerSimulator.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct SpeakerSimulatorMetaData {
		SN_NODE_ID("SpeakerSimulator");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using SpeakerSimulator = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _SpeakerSimulator, SpeakerSimulatorMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

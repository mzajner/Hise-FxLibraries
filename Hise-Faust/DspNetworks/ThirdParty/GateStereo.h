...
#include "src/GateStereo.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct GateStereoMetaData {
		SN_NODE_ID("GateStereo");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using GateStereo = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _GateStereo, GateStereoMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

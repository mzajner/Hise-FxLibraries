...
#include "src/mp3Compressor.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct mp3CompressorMetaData {
		SN_NODE_ID("mp3Compressor");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using mp3Compressor = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _mp3Compressor, mp3CompressorMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

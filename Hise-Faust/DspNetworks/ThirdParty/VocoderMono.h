...
#include "src/VocoderMono.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct VocoderMonoMetaData {
		SN_NODE_ID("VocoderMono");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using VocoderMono = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _VocoderMono, VocoderMonoMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

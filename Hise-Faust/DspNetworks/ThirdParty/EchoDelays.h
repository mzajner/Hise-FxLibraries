...
#include "src/EchoDelays.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct EchoDelaysMetaData {
		SN_NODE_ID("EchoDelays");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using EchoDelays = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _EchoDelays, EchoDelaysMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

...
#include "src/DiodeLadderLpf2Poles.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct DiodeLadderLpf2PolesMetaData {
		SN_NODE_ID("DiodeLadderLpf2Poles");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using DiodeLadderLpf2Poles = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _DiodeLadderLpf2Poles, DiodeLadderLpf2PolesMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

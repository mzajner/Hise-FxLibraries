...
#include "src/RingModulationMono.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct RingModulationMonoMetaData {
		SN_NODE_ID("RingModulationMono");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using RingModulationMono = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _RingModulationMono, RingModulationMonoMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

...
#include "src/OberheimHpf4Poles.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct OberheimHpf4PolesMetaData {
		SN_NODE_ID("OberheimHpf4Poles");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using OberheimHpf4Poles = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _OberheimHpf4Poles, OberheimHpf4PolesMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

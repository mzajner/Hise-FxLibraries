...
#include "src/OberheimLpf2Poles.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct OberheimLpf2PolesMetaData {
		SN_NODE_ID("OberheimLpf2Poles");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using OberheimLpf2Poles = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _OberheimLpf2Poles, OberheimLpf2PolesMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

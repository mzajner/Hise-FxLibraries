...
#include "src/EchoplexDelay.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct EchoplexDelayMetaData {
		SN_NODE_ID("EchoplexDelay");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using EchoplexDelay = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _EchoplexDelay, EchoplexDelayMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

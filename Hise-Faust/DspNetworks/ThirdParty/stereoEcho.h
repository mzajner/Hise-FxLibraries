...
#include "src/stereoEcho.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct stereoEchoMetaData {
		SN_NODE_ID("stereoEcho");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using stereoEcho = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _stereoEcho, stereoEchoMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

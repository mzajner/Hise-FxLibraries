...
#include "src/smoothDelay.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct smoothDelayMetaData {
		SN_NODE_ID("smoothDelay");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using smoothDelay = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _smoothDelay, smoothDelayMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

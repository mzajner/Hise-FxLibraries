...
#include "src/FuzzTone.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct FuzzToneMetaData {
		SN_NODE_ID("FuzzTone");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using FuzzTone = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _FuzzTone, FuzzToneMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

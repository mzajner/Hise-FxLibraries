...
#include "src/coLimiter1176stereo.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct coLimiter1176stereoMetaData {
		SN_NODE_ID("coLimiter1176stereo");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using coLimiter1176stereo = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _coLimiter1176stereo, coLimiter1176stereoMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

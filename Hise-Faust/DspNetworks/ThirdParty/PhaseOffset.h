...
#include "src/PhaseOffset.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct PhaseOffsetMetaData {
		SN_NODE_ID("PhaseOffset");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using PhaseOffset = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _PhaseOffset, PhaseOffsetMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

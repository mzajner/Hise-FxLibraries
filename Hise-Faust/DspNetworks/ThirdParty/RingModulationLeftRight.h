...
#include "src/RingModulationLeftRight.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct RingModulationLeftRightMetaData {
		SN_NODE_ID("RingModulationLeftRight");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using RingModulationLeftRight = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _RingModulationLeftRight, RingModulationLeftRightMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

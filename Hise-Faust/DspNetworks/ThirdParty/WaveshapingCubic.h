...
#include "src/WaveshapingCubic.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct WaveshapingCubicMetaData {
		SN_NODE_ID("WaveshapingCubic");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using WaveshapingCubic = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _WaveshapingCubic, WaveshapingCubicMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

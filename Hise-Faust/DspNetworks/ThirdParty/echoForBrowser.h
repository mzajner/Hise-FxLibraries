...
#include "src/echoForBrowser.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct echoForBrowserMetaData {
		SN_NODE_ID("echoForBrowser");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using echoForBrowser = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _echoForBrowser, echoForBrowserMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

...
#include "src/AllPassLoopReverb.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct AllPassLoopReverbMetaData {
		SN_NODE_ID("AllPassLoopReverb");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using AllPassLoopReverb = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _AllPassLoopReverb, AllPassLoopReverbMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

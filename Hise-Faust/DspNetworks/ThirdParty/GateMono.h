...
#include "src/GateMono.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct GateMonoMetaData {
		SN_NODE_ID("GateMono");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using GateMono = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _GateMono, GateMonoMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

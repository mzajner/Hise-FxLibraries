...
#include "src/Bitcrush.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct BitcrushMetaData {
		SN_NODE_ID("Bitcrush");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using Bitcrush = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _Bitcrush, BitcrushMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

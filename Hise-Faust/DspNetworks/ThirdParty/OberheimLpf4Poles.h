...
#include "src/OberheimLpf4Poles.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct OberheimLpf4PolesMetaData {
		SN_NODE_ID("OberheimLpf4Poles");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using OberheimLpf4Poles = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _OberheimLpf4Poles, OberheimLpf4PolesMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

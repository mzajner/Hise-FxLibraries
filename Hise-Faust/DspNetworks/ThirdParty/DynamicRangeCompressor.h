...
#include "src/DynamicRangeCompressor.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct DynamicRangeCompressorMetaData {
		SN_NODE_ID("DynamicRangeCompressor");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using DynamicRangeCompressor = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _DynamicRangeCompressor, DynamicRangeCompressorMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

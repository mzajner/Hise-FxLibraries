...
#include "src/DownSampler.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct DownSamplerMetaData {
		SN_NODE_ID("DownSampler");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using DownSampler = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _DownSampler, DownSamplerMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

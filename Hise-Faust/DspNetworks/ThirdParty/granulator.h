...
#include "src/granulator.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct granulatorMetaData {
		SN_NODE_ID("granulator");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using granulator = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _granulator, granulatorMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

...
#include "src/OberheimBpf4Poles.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct OberheimBpf4PolesMetaData {
		SN_NODE_ID("OberheimBpf4Poles");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using OberheimBpf4Poles = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _OberheimBpf4Poles, OberheimBpf4PolesMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

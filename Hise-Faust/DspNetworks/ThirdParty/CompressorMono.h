...
#include "src/CompressorMono.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct CompressorMonoMetaData {
		SN_NODE_ID("CompressorMono");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using CompressorMono = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _CompressorMono, CompressorMonoMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

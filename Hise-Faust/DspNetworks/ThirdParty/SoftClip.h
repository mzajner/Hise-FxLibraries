...
#include "src/SoftClip.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct SoftClipMetaData {
		SN_NODE_ID("SoftClip");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using SoftClip = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _SoftClip, SoftClipMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros

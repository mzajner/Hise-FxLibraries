...
//-------------------- MAIN -------------------------------
process = pitchshifter_drywet, pitchshifter_drywet;
...
pitchshifter =  transpose(w,x,s)
		with {
			w = hslider("window [units (ms)]", 75, 10, 1000, 1)* ma.SR*(0.001):si.smoo;
			//x = hslider("xfade [units (ms)]", 10, 1, 500, 1)*ma.SR*0.001 : smooth (0.99);
			x = w * 0.5:si.smoo;
			s = (hslider("v:sfTrashShift parameter(s)/shift [units (cents)] [acc:0 1 -10 0 10][color: 255 0 0 ][hidden:1]", 0, -100, 100, 0.1))*0.01 : si.smooth (0.998);
			};

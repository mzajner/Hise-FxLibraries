...
attack		= env_group(hslider("attack (ms)", 2, 0, 1000, 1)) : max(1/(SR*1000)) : /(1000);
release		= env_group(hslider("release (ms)", 500, 0, 3000, 10)) : max(1/(SR*1000)) : /(1000);

// gain controls: make-up gain, compression gain meter
makeup_gain	= gain_group(hslider("makeupGain", 0, -12, 12, 0.1));

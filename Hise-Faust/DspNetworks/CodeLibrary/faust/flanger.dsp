...
with{
    ...
    dmax = 2048;
    dflange = 0.001 * ma.SR *
        del_group(hslider("[1] Flange Delay [unit:ms] [style:knob]", 10, 0, 20, 0.001):si.smoo);
    odflange = 0.001 * ma.SR *
    del_group(hslider("[2] Delay Offset [unit:ms] [style:knob]", 1, 0, 20, 0.001):si.smoo);
    freq   = ctl_group(hslider("[1] Speed [unit:Hz] [style:knob]", 0.5, 0, 10, 0.01):si.smoo);
    depth  = ctl_group(hslider("[2] Depth [style:knob]", 1, 0, 1, 0.001):si.smoo);
    fb     = ctl_group(hslider("[3] Feedback [style:knob]", 0, -0.95, 0.95, 0.001):si.smoo);
    level  = lvl_group(hslider("Flanger Output Level [unit:dB]", 0, -60, 10, 0.1):si.smoo) :
        ba.db2linear;

process = vgroup("WahWah",environment{declare name "WahWah";
...
process = (_<:_,ve.crybaby(wah):drywet), (_<:_,ve.crybaby(wah):drywet)
	with {
   		wah = hslider("[1]Wah Wah[acc:0 1 -10 0 10]", 0.6,0,1,0.01) : ba.automat(bps, 15, 0.0);
   		bps = hslider("[2]Speed[acc:0 1 -10 0 10]", 540, 10, 780, 0.1):si.smooth(0.999):min(10):max(360):int;

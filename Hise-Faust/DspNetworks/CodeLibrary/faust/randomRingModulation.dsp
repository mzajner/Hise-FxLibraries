process = vgroup("RandomRingModulation",environment{declare name "Random Ring Modulator";
...
ringSpeed = hslider("[1]Ring Modulation Speed Range[scale:log][acc:2 0 -10 0 10]", 20, 0, 10000, 1) : si.smooth(0.99) : min(10000) : max(1);
...
proba = hslider ("[3]Probability(Granulator)[unit:%][style:knob][acc:0 1 -10 0 10]", 88,1,150,1) *(0.01):fi.lowpass(1,1);

...
// Reverb Time
KRT = DRYGroup(hslider("[1] KRT Decay [style: knob]",.8, 0, 0.98, .001) : si.smoo);
...
// Shimmer Amplitude
FBMIX = ShimmerGroup(hslider("[5] Shimmer [style: knob]", 0.5, 0.01, 0.99, .001) : si.smoo);
// Shimmer Frequency
FTUNE = ShimmerGroup(hslider("[7] Fine Tune [style: knob]", 0, -1, 1, .001) : si.smoo);
SHIFT = ShimmerGroup(hslider("[6] Frequency [style: knob]", 12, -24, 24, 0.1) : si.smoo);
// Input Signal

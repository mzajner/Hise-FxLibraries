/*
AspikDynamicsFeedback - Feedback Topology Dynamics Processor
Implements feedback topology where detector processes the output signal
...
namespace project
{
	...
	template <int NV> struct AspikDynamicsFeedback : public data::base
	{
		SNEX_NODE(AspikDynamicsFeedback);

		struct MetadataClass
		{
			SN_NODE_ID("AspikDynamicsFeedback");
		};
		...
		AspikDynamicsFeedbackProcessor processors[2][NV];
		...
		{
			int voiceIndex = 0; // Always use voice 0 - framework handles polyphony elsewhere
		...
		template <int P> void setParameter(double v)
		{
			if (P == AlgorithmType)
			{
				...
				for (int i = 0; i < NV; i++)
				{
					...
					case 1:
						...
						break;
					case 2:
						params.calculation = dynamicsProcessorType::kDownwardExpander;
						params.hardLimitGate = false;
						break;
					case 3:
						params.calculation = dynamicsProcessorType::kDownwardExpander;
			...
			else if (P == FeedbackGain)
			{
				for (int i = 0; i < NV; i++)
				{
					auto params = processors[0][i].getParameters();
					params.feedbackGain = v;
					processors[0][i].setParameters(params);
					processors[1][i].setParameters(params);
				}
			}
		...
		void createParameters(ParameterDataList& data)
		{
			...
			{
				parameter::data p("FeedbackGain", { 0.0, 1.0 });
				registerCallback<FeedbackGain>(p);
	...
	private:
		void updateParameters(int voiceIndex)
		{
			...
			params.ratio = 4.0;
			params.kneeWidth_dB = 5.0;
			params.attackTime_mSec = 5.0;
			params.releaseTime_mSec = 500.0;
			params.outputGain_dB = 0.0;
			params.hardLimitGate = false;
			params.softKnee = true;
			params.feedbackGain = 0.8;

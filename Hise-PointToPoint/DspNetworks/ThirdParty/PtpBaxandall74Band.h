...
template <int NV>
struct PtpBaxandall74Band : public data::base
{
    SNEX_NODE(PtpBaxandall74Band);

    struct MetadataClass { SN_NODE_ID("PtpBaxandall74Band"); };
    ...
    PointToPoint::Baxandall74Band effect;
    ...
    {
        {
            parameter::data p("Low", { 0.0, 1.0 });
            registerCallback<0>(p);
            p.setDefaultValue(0.5);
            data.add(std::move(p));
        }
        {
            parameter::data p("Low Mid", { 0.0, 1.0 });
            registerCallback<1>(p);
            p.setDefaultValue(0.5);
            data.add(std::move(p));
        }
        {
            parameter::data p("High Mid", { 0.0, 1.0 });

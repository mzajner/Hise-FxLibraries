...
template <int NV>
struct PtpDODFX70CToneStage : public data::base
{
    SNEX_NODE(PtpDODFX70CToneStage);

    struct MetadataClass { SN_NODE_ID("PtpDODFX70CToneStage"); };
    ...
    PointToPoint::DOD_FX70C_ToneStage effect;
    ...
    {
        {
            parameter::data p("Bottom", { 0.0, 1.0 });
            registerCallback<0>(p);
            p.setDefaultValue(0.5);
            data.add(std::move(p));
        }
        {
            parameter::data p("Top", { 0.0, 1.0 });
            registerCallback<1>(p);
            p.setDefaultValue(0.5);
            data.add(std::move(p));
        }
        {
            parameter::data p("Dissolve", { 0.0, 1.0 });

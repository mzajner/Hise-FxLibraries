...
template <int NV>
struct PtpHPFLPFCombo : public data::base
{
    SNEX_NODE(PtpHPFLPFCombo);

    struct MetadataClass { SN_NODE_ID("PtpHPFLPFCombo"); };
    ...
    PointToPoint::HPFLPFCombo effect;
    ...
    {
        {
            parameter::data p("HPF Cutoff", { 0.0, 1.0 });
            registerCallback<0>(p);
            p.setDefaultValue(0.5);
            data.add(std::move(p));
        }
        {
            parameter::data p("LPF Cutoff", { 0.0, 1.0 });

...
#include "src/PointToPoint/AmpTonestacks.h"
...
template <int NV>
struct PtpRolandJazzChorusToneStack : public data::base
{
    SNEX_NODE(PtpRolandJazzChorusToneStack);

    struct MetadataClass { SN_NODE_ID("PtpRolandJazzChorusToneStack"); };
    ...
    PointToPoint::RolandJazzChorusToneStack effect;

    double sampleRate = 44100.0;
    int    blockSize  = 512;

    std::vector<double> params { 0.5, 0.5, 0.5, 0.5, 0.5 };
    ...
    {
        {
            parameter::data p("Bass", { 0.0, 1.0 });
            registerCallback<0>(p);
            p.setDefaultValue(0.5);
            data.add(std::move(p));
        }
        {
            parameter::data p("Mid", { 0.0, 1.0 });
            registerCallback<1>(p);
            p.setDefaultValue(0.5);
            data.add(std::move(p));
        }
        {
            parameter::data p("Treble", { 0.0, 1.0 });
            registerCallback<2>(p);
            p.setDefaultValue(0.5);
            data.add(std::move(p));
        }
        {
            parameter::data p("High", { 0.0, 1.0 });

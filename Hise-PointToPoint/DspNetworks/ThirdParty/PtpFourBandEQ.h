...
template <int NV>
struct PtpFourBandEQ : public data::base
{
    SNEX_NODE(PtpFourBandEQ);

    struct MetadataClass { SN_NODE_ID("PtpFourBandEQ"); };
    ...
    PointToPoint::FourBandEQ effect;
    ...
    {
        {
            parameter::data p("Bass Frequency", { 0.0, 1.0 });
            registerCallback<0>(p);
            p.setDefaultValue(0.5);
            data.add(std::move(p));
        }
        {
            parameter::data p("Bass Gain", { 0.0, 1.0 });
            registerCallback<1>(p);
            p.setDefaultValue(0.5);
            data.add(std::move(p));
        }
        {
            parameter::data p("Low Mid Frequency", { 0.0, 1.0 });
            registerCallback<2>(p);
            p.setDefaultValue(0.5);
            data.add(std::move(p));
        }
        {
            parameter::data p("Low Mid Gain", { 0.0, 1.0 });
            registerCallback<3>(p);
            p.setDefaultValue(0.5);
            data.add(std::move(p));
        }
        {
            parameter::data p("High Mid Frequency", { 0.0, 1.0 });
            registerCallback<4>(p);
            p.setDefaultValue(0.5);
            data.add(std::move(p));
        }
        {
            parameter::data p("High Mid Gain", { 0.0, 1.0 });
            registerCallback<5>(p);
            p.setDefaultValue(0.5);
            data.add(std::move(p));
        }
        {
            parameter::data p("Treble Gain", { 0.0, 1.0 });
